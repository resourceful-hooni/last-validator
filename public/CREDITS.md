...
- **감정 낭독(영어 한정): Kokoro TTS(Apache-2.0 오픈웨이트)로 자작 생성**한 mp3(`public/audio/narration/en/`, 총 ~1.4MB).
  ...
  - 한국어/중/일은 오픈 감정 TTS 부재로 낭독 없음(무음 폴백). 옵트인 토글(기본 OFF, en 로케일)·음소거 연동·자막(화면 텍스트) 상시. 씬별 클립만 지연 로드.
  - 라이선스: Kokoro 모델 **Apache-2.0**(오픈웨이트, 상업 사용 가능). CC0/OFL은 아니며 **생성물은 자작 사운드**로 취급.
